...
test('should pass payload', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orUnsupportedMediaType(undefined, payload);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should pass message', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orUnsupportedMediaType(message);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should throw a "Unsupported Media Type" error code', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orUnsupportedMediaType();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.statusCode, CODES.UNSUPPORTED_MEDIA_TYPE);
  }
});

test('should throw a "Unsupported Media Type" error message when not set', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orUnsupportedMediaType();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.message, 'Unsupported Media Type');
  }
});
