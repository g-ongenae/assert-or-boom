...
test('should pass payload', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orPreconditionRequired(undefined, payload);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should pass message', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orPreconditionRequired(message);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should throw a "Precondition Required" error code', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orPreconditionRequired();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.statusCode, CODES.PRECONDITION_REQUIRED);
  }
});

test('should throw a "Precondition Required" error message when not set', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orPreconditionRequired();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.message, 'Precondition Required');
  }
});
