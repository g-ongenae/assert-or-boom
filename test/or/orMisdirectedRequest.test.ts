...
test('should pass payload', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orMisdirectedRequest(undefined, payload);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should pass message', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orMisdirectedRequest(message);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should throw a "Misdirected Request" error code', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orMisdirectedRequest();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.statusCode, CODES.MISDIRECTED_REQUEST);
  }
});

test('should throw a "Misdirected Request" error message when not set', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orMisdirectedRequest();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.message, 'Misdirected Request');
  }
});
