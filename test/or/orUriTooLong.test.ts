...
test('should pass payload', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orUriTooLong(undefined, payload);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should throw a "URI Too Long" error code', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orUriTooLong();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.statusCode, CODES.URI_TOO_LONG);
  }
});

test('should throw a "URI Too Long" error message when not set', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orUriTooLong();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.message, 'URI Too Long');
  }
});
