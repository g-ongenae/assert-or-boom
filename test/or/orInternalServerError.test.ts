...
test('should pass payload', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orInternalServerError(undefined, payload);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should pass message', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orInternalServerError(message);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should throw a "Internal Server Error" error code', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orInternalServerError();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.statusCode, CODES.INTERNAL_SERVER_ERROR);
  }
});

test('should throw a "Internal Server Error" error message when not set', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orInternalServerError();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.message, 'Internal Server Error');
  }
});
