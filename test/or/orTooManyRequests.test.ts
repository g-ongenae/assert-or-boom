...
test('should pass payload', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orTooManyRequests(undefined, payload);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should pass message', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orTooManyRequests(message);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should throw a "Too Many Requests" error code', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orTooManyRequests();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.statusCode, CODES.TOO_MANY_REQUESTS);
  }
});

test('should throw a "Too Many Requests" error message when not set', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orTooManyRequests();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.message, 'Too Many Requests');
  }
});
