...
test('should pass payload', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orLengthRequired(undefined, payload);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should pass message', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orLengthRequired(message);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should throw a "Length Required" error code', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orLengthRequired();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.statusCode, CODES.LENGTH_REQUIRED);
  }
});

test('should throw a "Length Required" error message when not set', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orLengthRequired();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.message, 'Length Required');
  }
});
