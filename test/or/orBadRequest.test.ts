...
test('should pass payload', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orBadRequest(undefined, payload);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should throw a "Bad Request" error code', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orBadRequest();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.statusCode, CODES.BAD_REQUEST);
  }
});

test('should throw a "Bad Request" error message when not set', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orBadRequest();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.message, 'Bad Request');
  }
});
