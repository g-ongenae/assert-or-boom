...
test('should pass payload', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orNotAcceptable(undefined, payload);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should pass message', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orNotAcceptable(message);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should throw a "Not Acceptable" error code', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orNotAcceptable();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.statusCode, CODES.NOT_ACCEPTABLE);
  }
});

test('should throw a "Not Acceptable" error message when not set', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orNotAcceptable();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.message, 'Not Acceptable');
  }
});
