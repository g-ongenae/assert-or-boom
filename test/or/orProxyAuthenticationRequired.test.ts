...
test('should pass payload', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orProxyAuthenticationRequired(undefined, payload);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should pass message', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orProxyAuthenticationRequired(message);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should throw a "Proxy Authentication Required" error code', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orProxyAuthenticationRequired();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.statusCode, CODES.PROXY_AUTHENTICATION_REQUIRED);
  }
});

test('should throw a "Proxy Authentication Required" error message when not set', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orProxyAuthenticationRequired();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.message, 'Proxy Authentication Required');
  }
});
