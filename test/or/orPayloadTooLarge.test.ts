...
test('should pass payload', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orPayloadTooLarge(undefined, payload);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should pass message', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orPayloadTooLarge(message);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should throw a "Payload Too Large" error code', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orPayloadTooLarge();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.statusCode, CODES.PAYLOAD_TOO_LARGE);
  }
});

test('should throw a "Payload Too Large" error message when not set', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orPayloadTooLarge();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.message, 'Payload Too Large');
  }
});
