...
test('should pass payload', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orLocked(undefined, payload);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should throw a "Locked" error code', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orLocked();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.statusCode, CODES.LOCKED);
  }
});

test('should throw a "Locked" error message when not set', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orLocked();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.message, 'Locked');
  }
});
