...
test('should pass payload', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orRequestTimeout(undefined, payload);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should pass message', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orRequestTimeout(message);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should throw a "Request Timeout" error code', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orRequestTimeout();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.statusCode, CODES.REQUEST_TIMEOUT);
  }
});

test('should throw a "Request Timeout" error message when not set', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orRequestTimeout();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.message, 'Request Timeout');
  }
});
