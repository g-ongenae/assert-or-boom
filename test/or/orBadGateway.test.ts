...
test('should pass payload', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orBadGateway(undefined, payload);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should throw a "Bad Gateway" error code', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orBadGateway();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.statusCode, CODES.BAD_GATEWAY);
  }
});

test('should throw a "Bad Gateway" error message when not set', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orBadGateway();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.message, 'Bad Gateway');
  }
});
