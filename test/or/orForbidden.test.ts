...
test('should throw a "Forbidden" error code', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orForbidden();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.statusCode, CODES.FORBIDDEN);
  }
});

test('should throw a "Forbidden" error message when not set', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orForbidden();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.message, 'Forbidden');
  }
});
