...
test('should pass payload', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orConflict(undefined, payload);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should throw a "Conflict" error code', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orConflict();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.statusCode, CODES.CONFLICT);
  }
});

test('should throw a "Conflict" error message when not set', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orConflict();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.message, 'Conflict');
  }
});
