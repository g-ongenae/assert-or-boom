...
test('should pass payload', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orUpgradeRequired(undefined, payload);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should pass message', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orUpgradeRequired(message);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should throw a "Upgrade Required" error code', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orUpgradeRequired();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.statusCode, CODES.UPGRADE_REQUIRED);
  }
});

test('should throw a "Upgrade Required" error message when not set', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orUpgradeRequired();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.message, 'Upgrade Required');
  }
});
