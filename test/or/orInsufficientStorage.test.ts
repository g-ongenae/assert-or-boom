...
test('should pass payload', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orInsufficientStorage(undefined, payload);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should pass message', (t: Assertions) => {
  ...
  assert.willThrow = true;
  try {
    assert.orInsufficientStorage(message);
    t.fail("Didn't throw an error");
  } catch (err) {
...
test('should throw a "Insufficient Storage" error code', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orInsufficientStorage();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.statusCode, CODES.INSUFFICIENT_STORAGE);
  }
});

test('should throw a "Insufficient Storage" error message when not set', (t: Assertions) => {
  assert.willThrow = true;
  try {
    assert.orInsufficientStorage();
    t.fail("Didn't throw an error");
  } catch (err) {
    t.is(err.output.message, 'Insufficient Storage');
  }
});
